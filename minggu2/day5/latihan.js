const menuMakanan = {
    ...
    salinMenu: function (...pilihId) {
        const tidakAda = pilihId.filter(id => !this.data.some(item => item.id === id))
        if (tidakAda.length > 0) { 
            console.log(`ID ${tidakAda} tidak ditemukan disini`)
            return []
        }
...
let hasil = menuMakanan.salinMenu()
console.log(hasil)
